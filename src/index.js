...
const makeRequest = (query) => {
  return fetch(url, {
    method: 'POST', // всегда для GraphQL
    headers: {
      "Content-type": "application/json" // всегда для GraphQL
    },
    body: JSON.stringify({ query }) // query всегда для GraphQL
  }).then(res => res.json())
}
...
async function handleTodoStatus() {
  ...
  // собственно сама GraphQL строка настраисаемая в plaseholders
